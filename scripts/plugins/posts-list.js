"use strict";

let path = require("path"),
    _ = require("lodash"),
    utils = require("../utils");

module.exports = function({posts, options, add, template, data}) {
    let pages = _.chunk(posts, options.posts.limit),
        indexPath = `${options.posts.index}${options.site.pretty_url ? "/index" : "" }.html`,
        ajaxPath = `${options.posts.index}${options.site.pretty_url ? "/" : "-" }index.json`,
        indexPage = pages.shift();

    const LAYOUT = path.join(options.location.layouts, options.posts.index_layout || options.posts.pagination_layout);

    function getPageNumLink(num) {
        return `${options.posts.pagination_dir.replace(":num", num)}${options.site.pretty_url ? "/index" : "" }.html`
    }

    function getAjaxPageNumLink(num) {
        return `${options.posts.pagination_dir.replace(":num", num)}${options.site.pretty_url ? "/index" : "" }.json`
    }

    function cleanPath(path) {
        return options.site.pretty_url ? path.replace(/index\.html$/, "") : path;
    }

    let indexPageData = _.merge({
        posts: indexPage,
        pages: {
            total: pages.length + 1,
            current: 1
        },
        links: {
            prev: false,
            next: pages.length > 0 ? cleanPath(getPageNumLink(2)) : false
        },
        title: "Blog"
    }, data);

    //Blog Index Page
    add(
        utils.createNewFile(
            path.join(...indexPath.split("/")),
            template.render(LAYOUT, indexPageData)
        )
    );

    indexPageData.ajax = true;

    add(
        utils.createNewFile(
            path.join(...ajaxPath.split("/")),
            template.render(LAYOUT, indexPageData)
        )
    )

    //Pagination pages >= 2
    pages.forEach((page, i) => {
        let num = i + 2,
            pageData = _.merge({
                posts: page,
                pages: {
                    total: pages.length + 1,
                    current: num,
                },
                links: {
                    prev: num === 2 ? cleanPath(indexPath) : cleanPath(getPageNumLink(num - 1)),
                    next: pages.length - 1 > i ? cleanPath(getPageNumLink(num + 1)) : false
                },
                title: `Page ${num} | Blog`
            }, data);

        add(
            utils.createNewFile(
                path.join(...getPageNumLink(num).split("/")),
                template.render(LAYOUT, pageData)
            )
        );

        pageData.ajax = true;

        add(
            utils.createNewFile(
                path.join(...getAjaxPageNumLink(num).split("/")),
                template.render(LAYOUT, pageData)
            )
        );
    })
};
